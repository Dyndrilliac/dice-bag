...
public final class Creature35E extends Creature
{
	...
	public final int compareTo(final Creature creature)
	{
		...
		{
			// By default, the creature with the highest total initiative acts first.
			// Turns then proceed in descending order of initiative.
		...
		else
		{
			...
			{
				// If two creatures have the same total initiative and initiative bonus,
				// then they both roll d20 tie-breakers until there is a winner.
				do
				{
	...
	public void updateStatus()
	{
			...
			&& (this.getStatBlock().getCurHealth() > this.getConstants().DISABLED_HP()))
		{
			...
			if (this.getStatBlock().getCurHealth() == this.getConstants().DISABLED_HP())
			{
				...
				if ((this.getStatBlock().getCurHealth() <= this.getConstants().DYING_HP())
					&& (this.getStatBlock().getCurHealth() > this.getConstants().DEAD_HP()))
				{
					if ((this.getStatBlock().getStatus() != Constants35E.Status.DYING) &&
						(this.getStatBlock().getStatus() != Constants35E.Status.UNCONCIOUS))
					{
						output.append(Color.BLACK, Color.WHITE, "[" + Support.getDateTimeStamp() + "]: ",
							Color.RED, Color.WHITE, this.getStatBlock().getName() +
					...
					if (this.getStatBlock().getCurHealth() <= this.getConstants().DEAD_HP())
					{
						output.append(Color.BLACK, Color.WHITE, "[" + Support.getDateTimeStamp() + "]: ",
							Color.RED, Color.WHITE, this.getStatBlock().getName() +
